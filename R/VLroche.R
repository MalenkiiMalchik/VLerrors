...
VLroche <- function(return = FALSE, download = FALSE, data){
...
test_activity_roche <- readr::read_csv(paste0(data, "/test_activity_roche.csv"))
...
error_codes = test_activity_roche %>%
  ...
  tidyr::separate(error_code_list, c("a","b","c","d","e","f","g","h", "i"), sep = ",") %>%
  tidyr::pivot_longer(cols = c("a","b","c","d","e","f","g","h", "i"), values_to = "error_code_list")
